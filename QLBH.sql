...
SELECT TOP 1 MONTH(NGHD), SUM(TRIGIA) FROM HOADON H
WHERE YEAR(NGHD) = 2006
GROUP BY MONTH(NGHD)
...
SELECT S.MASP, TENSP FROM SANPHAM S
WHERE S.MASP = (
	SELECT TOP 1 C.MASP FROM CTHD C
	GROUP BY C.MASP
	ORDER BY SUM(C.SL)
)
GO
...
SELECT NUOCSX, MAX(GIA) FROM SANPHAM
GROUP BY NUOCSX
--SAU DO DAT TEN BANG VUA ROI LA B, ROI THUC HIEN KET TRAI. OI HK THANH' VAI~ :))
SELECT B.NUOCSX, MASP, TENSP FROM (
	SELECT NUOCSX, MAX(GIA) AS MAX FROM SANPHAM
	GROUP BY NUOCSX
	) AS B JOIN SANPHAM S
ON B.NUOCSX = S.NUOCSX
WHERE B.MAX = S.GIA
GO
...
SELECT TOP 1 MAKH, COUNT(SOHD) AS SL 
FROM HOADON
GROUP BY MAKH
ORDER BY SL DESC
